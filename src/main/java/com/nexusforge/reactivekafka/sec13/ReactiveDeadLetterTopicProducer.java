...
public class ReactiveDeadLetterTopicProducer<K, V> {
    ...
    public Function<Mono<ReceiverRecord<K, V>>, Mono<Void>> recordProcessingErrorHandler() {
        return mono -> mono
                ...
                .onErrorMap(ex -> ex.getCause() instanceof RecordProcessingExcpetion, Throwable::getCause)
                .doOnError(ex -> ex.getMessage())
                .onErrorResume(RecordProcessingExcpetion.class, ex -> this.produce(ex.getRecord()).then(Mono
                        .fromRunnable(() -> ex.getRecord().receiverOffset().acknowledge()))).then();
